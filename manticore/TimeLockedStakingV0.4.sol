...
contract TimeLockedStaking is ERC165, ISimpleStaking {
  ...
  function getStakeRecordUnlockedAt(address user, bytes data) public view returns (uint256) {
    return stakers[user].stakeRecords[keccak256(data)].unlockedAt;
  }

  function getStakeRecordAmount(address user, bytes data) public view returns (uint256) {
